...
// PUT { whatsapp_numero?, direccion?, instagram_url?, facebook_url?, tiktok_url?, nuevo_codigo? }
// nuevo_codigo se hashea aquí, nunca se guarda en texto plano.
export async function PUT(req) {
  const guard = requireAdmin();
  if (guard) return guard;
  const { whatsapp_numero, direccion, instagram_url, facebook_url, tiktok_url, nuevo_codigo } = await req.json().catch(() => ({}));
  ...
  if (tiktok_url !== undefined) patch.tiktok_url = tiktok_url.trim() || null;
  if (nuevo_codigo) {
